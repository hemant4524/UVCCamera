 ...
 * on the device) now.
 */
public final class MainBackupActivity extends BaseActivity implements CameraDialog.CameraDialogParent {
			...
			case R.id.camera_view_L:
				...
				if (mHandlerL != null) {
					if (!mHandlerL.isOpened()) {
						CameraDialog.showDialog(MainBackupActivity.this);
					} else {
						mHandlerL.close();
						setCameraButton();
					}
...
//			}
		...
		public void onDettach(final UsbDevice device) {
			...
			Toast.makeText(MainBackupActivity.this, "USB_DEVICE_DETACHED", Toast.LENGTH_SHORT).show();
		}
